...
final class HomeServiceTest extends TestCase
{
    ...
    public function test_intake_submission_creates_home_service_request_for_home_dentistry(): void
    {
        ...
        $this->actingAs($user)->postJson("/api/v1/cases/{$caseId}/submit", ['version' => 1, 'policy_version' => 'home-approved-1', 'content_hash' => hash('sha256', 'متن رضایت')], ['Idempotency-Key' => 'home-submit-'.Str::uuid()])
            ->assertOk()
    ...
    public function test_intake_submission_does_not_create_home_service_for_non_home_dentistry(): void
    {
        ...
        $caseId = $draft->json('data.id');
        $this->actingAs($user)->postJson("/api/v1/cases/{$caseId}/submit", ['version' => 1, 'policy_version' => 'opg-approved-1', 'content_hash' => hash('sha256', 'متن رضایت')], ['Idempotency-Key' => 'opg-submit-'.Str::uuid()])
            ->assertOk();
