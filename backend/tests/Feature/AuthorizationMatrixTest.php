...
final class AuthorizationMatrixTest extends TestCase
{
    ...
    {
        $clinic = Clinic::query()->create(['name' => 'Clinic Granted', 'city' => 'Tehran']);
        $rep = User::factory()->create(['role' => 'clinic_rep']);
        ClinicMembership::query()->create([
            'clinic_id' => $clinic->id,
            'user_id' => $rep->id,
    ...
    public function test_intake_kill_switch_blocks_every_intake_route(): void
    {
        config()->set('royadarman.intake_enabled', false);
        $patient = User::factory()->create(['role' => 'patient', 'phone' => '09120000002', 'phone_hash' => hash('sha256', 'patient-2')]);

        $intakeRoutes = [
            ['post', '/api/v1/cases/draft', ['service_type' => 'guidance_referral', 'budget_band' => 'call', 'budget_input_unit' => 'toman', 'source_language' => 'fa']],
            ['post', '/api/v1/cases/'.$this->patientCase->id.'/submit', ['version' => 1, 'policy_version' => 'v1']],
            ['post', '/api/v1/cases/'.$this->patientCase->id.'/documents', []],
            ['post', '/api/v1/staff/cases/'.$this->patientCase->id.'/assignments', ['assignee_user_id' => $patient->id, 'purpose' => 'coordination', 'version' => 1]],
        ];

        foreach ($intakeRoutes as [$method, $uri, $body]) {
            $this->actingAs($patient)->{$method.'Json'}($uri, $body)
