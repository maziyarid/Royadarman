...
<header class="site-header">
    <div class="shell header-row">
        <a class="brand" href="/{{ $locale }}/" aria-label="{{ __('ui.meta_title') }}">
            ...
            <span><strong>{{ $locale === 'en' ? 'Royadarman' : 'رویاد درمان' }}</strong><small>{{ __('ui.brand_subtitle') }}</small></span>
        </a>
...
<footer><div class="shell footer-row"><div class="brand"><img src="/assets/brand-mark.svg" alt="" width="46" height="46"><span><strong>{{ $locale === 'en' ? 'Royadarman' : 'رویاد درمان' }}</strong><small>{{ __('ui.footer') }}</small></span></div><p>© {{ now()->setTimezone('Asia/Tehran')->format('Y') }}</p></div></footer>
</body>
</html>
