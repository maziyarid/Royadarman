@php
    ...
    $publicUrl = fn (string $name, string $locale): string => $locale === 'fa'
        ? route($name.'.fa')
...
<head>
    ...
    <link rel="canonical" href="{{ $publicUrl($routeName, app()->getLocale()) }}">
    @foreach(['fa','ar','en'] as $alternateLocale)
        <link rel="alternate" hreflang="{{ $alternateLocale }}" href="{{ $publicUrl($routeName, $alternateLocale) }}">
    @endforeach
    <style>
...
<main class="shell">
    <nav class="nav">
        <a class="brand" href="{{ $publicUrl('public.home', app()->getLocale()) }}">Royadarman</a>
        <div class="links">
            <a href="{{ $publicUrl('public.opg', app()->getLocale()) }}">OPG</a>
            <a href="{{ $publicUrl('public.home-dentistry', app()->getLocale()) }}">{{ app()->getLocale() === 'en' ? 'Home dentistry' : (app()->getLocale() === 'ar' ? 'طب الأسنان المنزلي' : 'دندانپزشکی در منزل') }}</a>
            <a href="{{ $publicUrl('public.referrals', app()->getLocale()) }}">{{ app()->getLocale() === 'en' ? 'Referrals' : (app()->getLocale() === 'ar' ? 'الإحالات' : 'معرفی مراکز') }}</a>
            <a href="{{ $publicUrl('public.contact', app()->getLocale()) }}">{{ app()->getLocale() === 'en' ? 'Contact' : (app()->getLocale() === 'ar' ? 'تواصل' : 'تماس') }}</a>
        </div>
    </nav>
