...
@section('content')
<h2 class="section-title">{{ __('ui.admin.comments') }}</h2>
...
<div class="card">
    ...
    <table>
        ...
        @foreach($comments as $c)
            ...
            <tr><td>{{ $c->author_name ?? ($c->author?->name ?? '—') }}</td><td class="compact-copy">{{ Str::limit($c->body, 100) }}</td>
            ...
            <td class="row-actions">
                ...
                <form method="POST" action="{{ route('admin.cms.comments.destroy', $c) }}">@csrf @method('DELETE')<button class="btn sm danger" type="submit" data-confirm="{{ __('ui.admin.delete_confirm') }}">{{ __('ui.admin.delete') }}</button></form>
            </td></tr>
        @endforeach
        </tbody>
    </table>
