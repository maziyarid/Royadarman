...
<div class="app-brand"><img src="/assets/brand-mark.svg" alt=""><div><strong>{{ __('panel.brand') }}</strong><span class="app-role">{{ __('panel.roles.'.$panelKey.'.title') }}</span></div></div>
...
@if($panelKey === 'patient')<a href="{{ route('patient.request.create',['locale'=>app()->getLocale()]) }}">{{ __('request.title') }}</a>@endif
...
<div class="nav-sep"></div><a href="{{ app()->getLocale()==='fa' ? route('public.home.fa') : route('public.home',['locale'=>app()->getLocale()]) }}">{{ __('panel.back_home') }}</a></nav>
...
<div class="app-main"><header class="app-topbar"><button class="btn mobile-nav" type="button" data-mobile-nav>☰</button><h1>{{ __('panel.roles.'.$panelKey.'.title') }}</h1><div class="app-top-actions">@if($panelKey==='patient')<a class="btn primary" href="{{ route('patient.request.create',['locale'=>app()->getLocale()]) }}">{{ __('request.title') }}</a>@endif</div></header>
<main class="app-content"><section class="hero-panel"><h1>{{ __('panel.roles.'.$panelKey.'.title') }}</h1><p>{{ __('panel.roles.'.$panelKey.'.subtitle') }}</p></section>
...
@if($cases->isNotEmpty())<section class="card"><div class="card-head"><span>{{ __('panel.roles.'.$panelKey.'.title') }}</span><span class="muted">{{ $cases->count() }}</span></div><div class="table-wrap"><table><thead><tr><th>{{ __('panel.table.reference') }}</th><th>{{ __('panel.table.service') }}</th><th>{{ __('panel.table.status') }}</th><th>{{ __('panel.table.updated') }}</th></tr></thead><tbody>@foreach($cases as $case)@php($service=$case->service_type instanceof \BackedEnum?$case->service_type->value:$case->service_type)@php($status=$case->status instanceof \BackedEnum?$case->status->value:$case->status)<tr><td><a class="case-link" href="{{ route('panel.case',['locale'=>app()->getLocale(),'case'=>$case->id]) }}"><bdi>{{ $case->public_reference }}</bdi></a></td><td><span class="badge">{{ $service }}</span></td><td><span class="badge {{ $status }}">{{ $status }}</span></td><td><bdi>{{ $case->updated_at }}</bdi></td></tr>@endforeach</tbody></table></div></section>@else<section class="card"><div class="empty">{{ __('panel.empty') }}</div></section>@endif
