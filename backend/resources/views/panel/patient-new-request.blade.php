...
@section('content')
@php
    $intakeEnabled = !empty($intakeEnabled);
    $isDemo = !empty($isDemo);
    $neighborhoods = $tehranNeighborhoods ?? [];
@endphp
<div
    data-patient-request
    data-locale="{{ $locale }}"
    data-intake="{{ $intakeEnabled ? '1' : '0' }}"
    data-demo="{{ $isDemo ? '1' : '0' }}"
    data-error="{{ __('request.error') }}"
    data-consent-unavailable="{{ __('request.consent_unavailable') }}"
    data-working="{{ __('request.working') }}"
    data-success="{{ __('request.success') }}"
    data-submit="{{ __('request.submit') }}"
    data-next="{{ __('request.next') }}"
    data-prev="{{ __('request.prev') }}"
    data-step-of="{{ __('request.step_of') }}"
    data-intake-closed="{{ __('request.intake_closed') }}"
    data-demo-readonly="{{ __('request.demo_readonly') }}"
>
    ...
    <section class="card pad request-wizard">
        ...
        <form id="request-form" novalidate>
            <div class="request-step" data-step="1">
                <h2 class="request-step-title">{{ __('request.steps.service') }}</h2>
                <p class="hint">{{ __('request.service_help') }}</p>
                <div class="request-choice-grid" role="radiogroup">
                    @foreach(['guidance_referral','opg_review','home_dentistry'] as $service)
                        <label class="request-choice">
                            <input type="radio" name="service_type" value="{{ $service }}" required @checked($loop->first)>
                            <span>
                                <strong>{{ __('request.services.'.$service) }}</strong>
                                <small>{{ __('request.service_hints.'.$service) }}</small>
                            </span>
                        </label>
                    @endforeach
                </div>
            ...
            <div class="request-step" data-step="2" hidden>
                <h2 class="request-step-title">{{ __('request.steps.urgency') }}</h2>
                <p class="hint">{{ __('request.urgency_help') }}</p>
                <div class="request-choice-grid" role="radiogroup">
                    @foreach(['normal'=>'routine','urgent'=>'urgent'] as $value=>$key)
                        <label class="request-choice">
                            <input type="radio" name="priority" value="{{ $value }}" required @checked($value==='normal')>
                            <span>
                                <strong>{{ __('request.urgencies.'.$key) }}</strong>
                                <small>{{ __('request.urgency_hints.'.$key) }}</small>
                            </span>
                        </label>
                    @endforeach
                </div>
                <div class="notice" data-escalate hidden>
                    <strong>{{ __('request.escalate_title') }}</strong>
                    <p>{{ __('request.escalate_text') }}</p>
                </div>
            </div>
                ...
                @if($neighborhoods)
                    ...
                    <div class="coverage-chips request-neighborhoods" role="group">
                        @foreach($neighborhoods as $n)
                            <button type="button" class="chip" data-neighborhood data-area="{{ $n['area'] }}">{{ $n[$locale] ?? $n['en'] }}</button>
                        @endforeach
                    </div>
                @endif
                <div class="field">
                    <label for="tehran_area">{{ __('request.tehran_area') }}</label>
                    <select id="tehran_area" name="tehran_area">
                        <option value="">{{ __('request.area_optional') }}</option>
                        @foreach($tehranAreas as $area)
                            <option value="{{ $area }}">{{ __('request.areas.'.$area) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="grid">
                    <div class="field">
                        ...
                        <select id="contact_time">
                            @foreach(['any','morning','midday','evening','night'] as $time)
                                <option value="{{ $time }}">{{ __('request.times.'.$time) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field">
                        ...
                        <select id="budget" required>
                            @foreach(['balanced','economic','flexible','call'] as $budget)
                                <option value="{{ $budget }}">{{ __('request.budgets.'.$budget) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="request-step" data-step="5" hidden>
                <h2 class="request-step-title">{{ __('request.steps.details') }}</h2>
                <div class="grid">
                    <div class="field">
                    ...
                    <div class="field full">
                        ...
                        <textarea id="reason" maxlength="1000" dir="auto" rows="4"></textarea>
                    </div>
                </div>
            </div>
            <div class="request-step" data-step="6" hidden>
                <h2 class="request-step-title">{{ __('request.steps.documents') }}</h2>
                <div class="notice">
                    <p>{{ __('request.documents_note') }}</p>
                    <p>{{ __('request.documents_no_diagnosis') }}</p>
                </div>
            </div>
            <div class="request-step" data-step="7" hidden>
                <h2 class="request-step-title">{{ __('request.steps.consent') }}</h2>
                <section class="consent" aria-labelledby="consent-title">
                    ...
                    <label class="check">
                        <input id="accept" type="checkbox" disabled required>
                        <span>{{ __('request.accept') }}</span>
                    </label>
                </section>
            </div>
            <div class="request-step" data-step="8" hidden>
                <h2 class="request-step-title">{{ __('request.steps.review') }}</h2>
                <p class="hint">{{ __('request.review_help') }}</p>
                <dl class="request-summary" data-summary>
                    ...
                    <div class="full"><dt>{{ __('request.reason') }}</dt><dd data-sum="reason">—</dd></div>
                </dl>
            </div>
            <div id="message" role="status" aria-live="polite"></div>
            <div class="actions request-nav">
                <button type="button" class="btn" data-prev hidden>{{ __('request.prev') }}</button>
                <button type="button" class="btn primary" data-next>{{ __('request.next') }}</button>
                <button id="submit" class="btn primary" type="submit" hidden disabled>{{ __('request.submit') }}</button>
                <a class="btn" href="{{ route('panel', ['locale' => $locale]) }}">{{ __('request.back') }}</a>
            </div>
        </form>
    </section>
