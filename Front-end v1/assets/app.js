    ...
    <header class="site-header ${compact ? "is-compact" : ""}">
      <div class="container header-inner">
        <a class="brand-lockup" href="#/home" aria-label="رویا درمان، صفحه اصلی">
          <span class="brand-symbol" aria-hidden="true">ر</span>
          <span class="brand-copy"><strong>رویا درمان</strong><small>راهنمایی و پشتیبانی دندانپزشکی</small></span>
        </a>
        ${compact ? `
        ...
        ` : `
          <nav class="site-nav" aria-label="فهرست اصلی">
            ...
            <a href="#process">مسیر همراهی</a>
            <a href="#trust">حریم و مسئولیت</a>
            <a href="#questions">پرسش‌ها</a>
          </nav>
          <a class="button button-primary header-cta" href="#/request/service">ثبت درخواست</a>
        `}
      </div>
    </header>
    ${content}
...
function renderHome() {
  ...
  document.title = "رویا درمان | راهنمایی و پشتیبانی شبانه‌روزی دندانپزشکی";
  app.innerHTML = shell(`
    <main id="main-content">
      <section class="hero">
        <div class="container hero-grid">
          <div class="hero-copy">
            <span class="eyebrow"><span class="live-dot"></span> پاسخ‌گویی و پیگیری در تمام ساعات شبانه‌روز</span>
            <h1>مرکز بیماریابی<sup>*</sup> و پشتیبانی <span>۲۴ ساعته</span> بیماران</h1>
            <p class="hero-lede">از اولین پرسش تا پیدا کردن مسیر مناسب کنارتان هستیم؛ برای هماهنگی خدمات در منزل در تهران، بررسی اولیه OPG و معرفی دندانپزشک یا مرکز متناسب با نیاز و بودجه شما.</p>
            <div class="hero-actions">
              <a class="button button-primary button-large" href="#/request/service">ثبت درخواست راهنمایی ${icon("arrow", "icon-sm")}</a>
              <a class="button button-secondary button-large" href="#services">مشاهده خدمات</a>
            </div>
            <p class="hero-footnote"><sup>*</sup> منظور از بیماریابی در رویا درمان، راهنمایی و هماهنگی برای یافتن مسیر مناسب است؛ تشخیص و درمان فقط توسط دندانپزشک دارای مجوز انجام می‌شود.</p>
          </div>
          <div class="route-card" aria-label="مسیر همراهی رویا درمان از درخواست تا معرفی">
            <div class="route-orbit" aria-hidden="true"></div>
            <div class="route-start"><span>شروع</span><strong>درخواست شما</strong></div>
            <ol class="route-steps">
              <li><span class="route-index">۱</span><div><strong>شنیدن نیاز</strong><small>تماس و دریافت اطلاعات ضروری</small></div></li>
              <li><span class="route-index">۲</span><div><strong>بررسی انسانی</strong><small>هماهنگ‌کننده و دندانپزشک در جای درست</small></div></li>
              <li><span class="route-index">۳</span><div><strong>معرفی و پیگیری</strong><small>گزینه مناسب، شفاف و قابل پیگیری</small></div></li>
            </ol>
            <div class="route-note">${icon("clock")}<span><strong>پشتیبانی شبانه‌روزی</strong><small>برای راهنمایی و پیگیری درخواست‌ها</small></span></div>
          </div>
        </div>
      </section>

      <section class="trust-strip" aria-label="تعهدهای اصلی">
        <div class="container trust-grid">
          <div>${icon("users")}<span><strong>شبکه مراکز همکار</strong><small>انتخاب متناسب با نیاز و شرایط شما</small></span></div>
          <div>${icon("shield")}<span><strong>مرز روشن مسئولیت</strong><small>نظر بالینی فقط توسط دندانپزشک مجاز</small></span></div>
          <div>${icon("location")}<span><strong>خدمت در منزل</strong><small>در مرحله فعلی، ویژه شهر تهران</small></span></div>
        </div>
      </section>
      ...
      <section class="section" id="services">
        <div class="container">
          <div class="section-head">
            <div><span class="section-kicker">سه مسیر روشن</span><h2>از کجا می‌خواهید شروع کنیم؟</h2></div>
            <p>یک مسیر را انتخاب کنید؛ تیم رویا درمان برای ادامه هماهنگی با شما تماس می‌گیرد.</p>
          </div>
          <div class="service-grid">
            ${Object.entries(serviceMeta).map(([key, item], index) => `
              <article class="service-card ${key === "opg" ? "is-featured" : ""}">
                ${key === "opg" ? '<span class="card-flag">ارسال تصویر</span>' : ""}
                <span class="service-number">۰${index + 1}</span>
                <span class="service-icon">${icon(item.icon)}</span>
                <h3>${item.title}</h3>
                <p>${item.description}</p>
                ${key === "home" ? '<span class="scope-label">پوشش فعلی: شهر تهران</span>' : ""}
                ${key === "opg" ? '<span class="scope-label">JPEG، PNG یا PDF</span>' : ""}
                <a class="text-action" href="#/request/contact" data-start-service="${key}">شروع این مسیر ${icon("arrow", "icon-sm")}</a>
              </article>
            `).join("")}
          </div>
        </div>
      ...
      <section class="section section-tint" id="process">
        <div class="container process-layout">
          <div class="process-copy">
            <span class="section-kicker">همراهی، نه سردرگمی</span>
            <h2>یک تیم انسانی، مسیر بعدی را با شما هماهنگ می‌کند</h2>
            <p>رویا درمان با دندانپزشکان و کلینیک‌های متعدد همکاری می‌کند. معرفی بر اساس تناسب خدمت، موقعیت، ظرفیت، نیازهای شما و بودجه اعلام‌شده انجام می‌شود؛ نه صرفاً یک فهرست تبلیغاتی.</p>
            <div class="boundary-note">${icon("shield")}<div><strong>تصمیم بالینی مستقل می‌ماند</strong><p>بودجه در انتخاب گزینه‌ها دیده می‌شود، اما هیچ انتخاب اقتصادی جای ایمنی و تشخیص حرفه‌ای را نمی‌گیرد.</p></div></div>
          </div>
          <ol class="process-list">
            <li><span>۱</span><div><strong>درخواست کوتاه</strong><p>نوع خدمت، راه تماس و ترجیحات ضروری را ثبت می‌کنید.</p></div></li>
            <li><span>۲</span><div><strong>پیگیری کارشناس</strong><p>هماهنگ‌کننده برای تکمیل اطلاعات و تعیین قدم مناسب تماس می‌گیرد.</p></div></li>
            <li><span>۳</span><div><strong>بررسی یا معرفی</strong><p>در صورت نیاز، تصویر به دندانپزشک دارای مجوز یا درخواست به مرکز منتخب ارجاع می‌شود.</p></div></li>
            <li><span>۴</span><div><strong>هماهنگی و پشتیبانی</strong><p>نتیجه، محدودیت‌ها و قدم بعدی روشن و قابل پیگیری اعلام می‌شود.</p></div></li>
          </ol>
        </div>
      </section>

      <section class="section" id="trust">
        <div class="container opg-layout">
          <div class="opg-visual" aria-hidden="true">
            <div class="film-frame">
              <span class="jaw jaw-top"></span><span class="jaw jaw-bottom"></span>
              <span class="film-mark">OPG</span>
            </div>
            <span class="secure-chip">${icon("lock", "icon-sm")} دسترسی محدود</span>
          </div>
          <div class="opg-copy">
            <span class="section-kicker">بررسی OPG با مرز روشن</span>
            <h2>تصویر شما برای نظر اولیه دندانپزشک است، نه تشخیص خودکار</h2>
            <p>پس از راه‌اندازی سامانه امن، فایل در فضای خصوصی دریافت، کنترل امنیتی و فقط در اختیار دندانپزشک تعیین‌شده قرار می‌گیرد. نتیجه با نام و مسئولیت همان دندانپزشک ثبت می‌شود.</p>
            <ul class="check-list">
              <li>${icon("check", "icon-sm")} پیشنهادهای اولیه با توجه به نیاز و بودجه اعلام‌شده</li>
              <li>${icon("check", "icon-sm")} دسترسی محدود و ثبت‌شده برای افراد مجاز</li>
              <li>${icon("check", "icon-sm")} توضیح محدودیت تصویر و قدم بعدی</li>
            </ul>
            <div class="clinical-limit"><strong>نکته بالینی</strong><p>بررسی تصویر جایگزین معاینه حضوری، تشخیص قطعی یا طرح درمان نهایی نیست.</p></div>
            <button class="button button-secondary" type="button" data-start-service="opg">پیش‌نمایش ارسال OPG</button>
          </div>
        </div>
      </section>

      <section class="section network-section">
        <div class="container network-card">
          <div>
            <span class="section-kicker section-kicker-light">یک مرکز، چندین امکان</span>
            <h2>معرفی از میان دندانپزشکان و کلینیک‌های همکار</h2>
            <p>سال‌ها همکاری با مراکز مختلف، امکان مقایسه عملی گزینه‌ها را فراهم می‌کند. معیار معرفی باید روشن، حرفه‌ای و متناسب با نیاز واقعی بیمار باشد.</p>
          </div>
          <div class="network-factors">
            <span>${icon("heart")} تناسب خدمت</span>
            <span>${icon("location")} محدوده و دسترسی</span>
            <span>${icon("wallet")} بودجه اعلام‌شده</span>
            <span>${icon("clock")} زمان و ظرفیت پاسخ</span>
          </div>
        </div>
      </section>
      ...
      <section class="section" id="questions">
        <div class="container faq-layout">
          <div class="faq-intro"><span class="section-kicker">پیش از شروع</span><h2>پرسش‌های مهم</h2><p>پاسخ‌های کوتاه درباره محدوده خدمت و مسئولیت رویا درمان.</p></div>
          <div class="faq-list">
            <details><summary>آیا رویا درمان خودش تشخیص یا درمان انجام می‌دهد؟ <span>+</span></summary><p>رویا درمان مسیر را هماهنگ و پشتیبانی می‌کند. تشخیص، نظر تخصصی و درمان فقط توسط دندانپزشک یا مرکز درمانی دارای مجوز انجام می‌شود.</p></details>
            <details><summary>خدمات دندانپزشکی در منزل کجا ارائه می‌شود؟ <span>+</span></summary><p>پوشش فعلی برای شهر تهران است. نوع خدمت قابل انجام در منزل پس از بررسی شرایط و طبق سیاست بالینی مشخص می‌شود.</p></details>
            <details><summary>آیا با ارسال OPG طرح درمان قطعی می‌گیرم؟ <span>+</span></summary><p>خیر. تصویر می‌تواند برای نظر و گزینه‌های اولیه کمک‌کننده باشد، اما طرح درمان نهایی معمولاً به معاینه و اطلاعات بالینی کامل نیاز دارد.</p></details>
            <details><summary>بودجه من چطور در معرفی مرکز دیده می‌شود؟ <span>+</span></summary><p>بودجه و ترجیحات شما کنار نیاز، موقعیت و امکان ارائه خدمت بررسی می‌شود. ایمنی و تناسب بالینی همواره مقدم است.</p></details>
            <details><summary>آیا پشتیبانی ۲۴ ساعته همان اورژانس است؟ <span>+</span></summary><p>خیر. این خدمت برای دریافت، راهنمایی و پیگیری درخواست‌هاست و جایگزین خدمات اورژانسی یا تماس با مراکز فوریت‌های پزشکی نیست.</p></details>
          </div>
        </div>
      </section>
      ...
      <section class="final-cta">
        <div class="container final-cta-inner">
          <div><span>قدم اول کوتاه است</span><h2>نیازتان را بگویید؛ مسیر بعدی را با هم روشن می‌کنیم.</h2><p>نسخه فعلی یک پیش‌نمایش تعاملی است و اطلاعات واقعی ارسال نمی‌کند.</p></div>
          <a class="button button-light button-large" href="#/request/service">شروع پیش‌نمایش درخواست ${icon("arrow", "icon-sm")}</a>
        </div>
      </section>
    </main>
    <footer class="site-footer">
      <div class="container footer-grid">
        <div><a class="brand-lockup footer-brand" href="#/home"><span class="brand-symbol">ر</span><span class="brand-copy"><strong>رویا درمان</strong><small>همراه مسیر درمان</small></span></a><p>مرکز راهنمایی، هماهنگی و پشتیبانی دندانپزشکی. خدمات بالینی توسط دندانپزشکان و مراکز دارای مجوز ارائه می‌شود.</p></div>
        <div><strong>خدمات</strong><a href="#services">راهنمایی و معرفی</a><a href="#services">خدمت در منزل تهران</a><a href="#trust">بررسی اولیه OPG</a></div>
        <div><strong>شفافیت</strong><a href="#trust">مرز مسئولیت بالینی</a><a href="#questions">پرسش‌های متداول</a><a href="/demo/" rel="nofollow">دموی قدیمی محصول</a></div>
      </div>
      <div class="container footer-bottom"><span>© ۱۴۰۵ رویا درمان</span><span>نسخه پیش‌نمایش؛ ثبت درخواست واقعی هنوز فعال نیست.</span></div>
    </footer>
    <a class="mobile-cta" href="#/request/service">ثبت درخواست</a>
  `);
  bindLandingAnchors();
}
