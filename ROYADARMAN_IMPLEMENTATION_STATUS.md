...
| Tests | VERIFIED (local) | 48 passed / 133 assertions across 11 files (OTP, case workflow, clinical access, document pipeline, locale+authz, operations, intake-disabled, error envelopes, **AuthorizationMatrixTest**, **SupportSystemTest**). PHP 8.4 local; Pint PASS. | Run on PHP 8.3 prod target + MariaDB. | Run on prod. |
