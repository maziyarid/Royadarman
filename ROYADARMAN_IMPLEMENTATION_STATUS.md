...
| Tests | VERIFIED (local) | 83 passed / 236 assertions across 17 files (OTP, case workflow, clinical access, document pipeline, locale+authz, operations, intake-disabled, error envelopes, **AuthorizationMatrixTest**, **SupportSystemTest**, **HomeServiceStatusTest**, **CmsPostTest**, **CmsContentTest**, **CmsManagementTest**). PHP 8.4 local; Pint PASS; composer audit clean. | Run on PHP 8.3 prod target + MariaDB. | Run on prod. |
