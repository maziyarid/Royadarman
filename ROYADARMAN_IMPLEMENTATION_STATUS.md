...
| SEO | VERIFIED (local) | `SitemapController` (sitemap index `sitemap.xml` + per-locale `sitemap-{fa,ar,en}.xml` from published posts/pages/services only; excludes drafts/admin/api/private media). `RobotsController` (`robots.txt` allows `/`, disallows `/api/`,`/admin/`,`/cms/`,`/storage/`, references sitemap). `BlogController` renders published CMS posts at `/{locale}/blog/{slug}` with per-locale canonical, hreflang (fa/ar/en + x-default), robots directive, OpenGraph/Twitter Card, JSON-LD `schema_data` from `cms_seo_metadata`. Home view already had canonical/hreflang/meta-description/robots. **Added:** `PublicRedirectController` (fallback route) wires redirect manager into public 404 path — matches unmatched GET paths against `cms_redirects` (active only), issues 301/302 with hit-count increment, real 404 for unknown paths (no redirect loops). `CmsMediaServeController` (`/cms-media/{media}`) serves public CMS marketing media from `public-cms` disk with immutable cache headers, image-only guard (non-image → 404). Fixed BlogController OG image bug (`$seo->ogImage->path` → `route('cms.media.serve', ...)`, was using nonexistent `path` column instead of `storage_key`). 11 feature tests (SeoPublicTest): robots+sitemap ref, sitemap index, locale sitemap published-only, blog canonical+hreflang, draft 404, redirect 301, redirect 302, inactive redirect 404, unknown path 404, CMS media served, non-image media 404. | **Structured data done:** Organization JSON-LD on home (via @php+json_encode to avoid Blade @context directive collision); Article JSON-LD on blog show via `StructuredDataService`. BreadcrumbList/FAQPage deferred until real content/structure lands. | Continue. |
