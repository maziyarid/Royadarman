...
| Public frontend | PARTIAL | `Current Public_HTML/` + `Front-end v1/` static multilingual frontend; `public/home.blade.php` minimal; approved identity in `public/assets/`. **Added:** `PageController::show` renders published CMS pages (type=Page) at `/{locale}/{slug}` and service pages (type=Service) at `/{locale}/services/{slug}`, with per-locale canonical, hreflang (fa/ar/en + x-default), OpenGraph, and JSON-LD structured data (`StructuredDataService::webPage()` / `service()`). 404 for drafts and non-Page types (blog posts stay on blog route — type isolation). Page routes ordered after dashboard/admin so the catch-all no longer shadows `/{locale}/dashboard` (fixed 8 DashboardUiTest 404s). `public/page/show.blade.php` view. 4 SeoPublicTest cases. | Approved visual design reconciliation (static vs Blade) still pending; WYSIWYG editor frontend. | Continue. |
