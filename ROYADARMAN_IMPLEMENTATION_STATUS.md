...
| Identity — Staff MFA | VERIFIED (local) | `TotpVerifier` (HOTP/TOTP, ±30s window, timing-safe compare); `mfa_recovery_codes` encrypted:array cast on User, consumed single-use on login. **Fixed critical bug:** Base32 alphabet was a REDACTED placeholder → restored RFC 4648 alphabet (would have broken all staff TOTP). 5 OtpAuthenticationTest cases: OTP normalise/hash/single-use, expired rejected, recovery-code consumed, TOTP valid-code + window tolerance, staff-with-totp requires valid TOTP or recovery (missing/wrong rejected). | Rapid revocation (admin disables user.is_active), idle/session-expiry controls to verify on production PHP 8.3. | Continue. |
...
| Clinical review | VERIFIED (local) | `ReviewRevision` model (encrypted fields, signed_at, supersedes/supersededBy versioning, isPublished) + `PublicationEvent` model (immutable, actor). Design uses versioned revisions + append-only publication events instead of a single `Review` parent (satisfies §16 append-only/correction). `StaffCaseController::createReview`/`publishReview` enforce: Clinician role + `can('view', $case)` (active clinical assignment + verified non-expired credential) + own-review ownership + 409 on re-publish. **6 ClinicalAccessTest cases** verify happy path, expired credential (404 non-enumerating), coordinator cannot publish (404), other clinician cannot publish (404), re-publish 409. | Runtime verify on MariaDB; review patient-visible publication route. | Continue. |
...
| Operations — Notifications | IMPLEMENTED_UNVERIFIED | `notification_deliveries` table, `HttpNotificationSender`, `NotificationCallbackController`. | SMS provider abstraction | IMPLEMENTED_UNVERIFIED (local) | `SmsProvider` contract (send, verifyCallback, parseCallback), `SmsDeliveryException` (notConfigured/providerFailed/quotaExceeded), `SmsManager` (provider registry + config selector `royadarman.sms.provider`), `HttpSmsProvider` generic HTTP impl (token auth, Idempotency-Key header, HMAC callback verification, 5-min timestamp window, 429 quota, retry-on-connection-error-only, failure status). `HttpNotificationSender` + `HttpOtpSender` refactored to delegate to SmsManager (no provider API shape in domain logic). `NotificationCallbackController` delegates signature verification + parsing to SmsProvider. 13 SmsProviderTest cases. | Runtime verify with live provider; production credentials Gate B external (BLOCKED_EXTERNAL: SMS_CREDENTIALS). | Activate when credentials approved. |
...
| Home dentistry | VERIFIED (local) | `ServiceType::HomeDentistry` enum + intake Tehran-area validation + **`HomeServiceStatus` state machine** (requested→area_verified→coordinator_review→provider_requested→provider_accepted→patient_confirmed→scheduled→completed; terminals: cancelled/rejected/unable_to_service) on `home_service_requests`/`home_service_status_events` tables; `HomeServiceRequest`/`HomeServiceStatusEvent` models; 4 unit tests. **`HomeServiceController`** (show, indexForCase, transition [coordinator/clinic-rep only, optimistic concurrency, state-machine validated, status events], patient confirm gated on provider_accepted). **`CaseController::submit` creates HomeServiceRequest at `requested`** for home_dentistry cases (inside submit transaction). 9 HomeServiceTest feature cases. Precise address NOT stored at request (only tehran_area). | Runtime verify on MariaDB. | Continue. |
| Support system | VERIFIED (local) | `support_conversations`/`support_messages`/`support_status_events` tables; `SupportConversation`/`SupportMessage`/`SupportStatusEvent` models; `ConversationStatus`/`SupportCategory`/`SupportPriority` enums; `SupportController`; `SupportConversationPolicy`; 11 feature tests (create, cross-patient denial, internal-note isolation, patient-cannot-internal-note, status state-machine + event, invalid transition rejected, scoped list, index visibility allowlist: clinician+clinic_rep denied 403 with no leak, owner sees all). Built fresh per §20. **Fixed (Greptile P1):** `SupportController::index` now enforces a deny-by-default allowlist matching `SupportConversationPolicy::view` — Patient (own), Coordinator (assigned), Owner/TechAdmin (all); Clinician/ClinicRep previously fell through to the unscoped query leaking every patient's conversation subject/category/status/priority/opened_at. | Runtime verify on MariaDB. | Deploy/migrate. |
| CMS / Marketing | PARTIAL | 16 CMS tables (posts, translations, categories, tags, media, menus, seo_metadata, redirects, comments, revisions), 16 Eloquent models, PostStatus/PostType/CommentStatus/MenuItemType enums, HtmlSanitizer (no stored XSS), CmsPostController (CRUD+publish+unpublish), CmsPostPolicy (owner/tech_admin/coordinator-editor; patient/clinician denied), 8 post feature tests. **Added:** CmsCategoryController (CRUD+show, hierarchical, self-parent prevented), CmsTagController (CRUD+merge, tag-merge moves post_tag pivot; self-merge rejected), CmsRedirectController (CRUD, 409 on duplicate source_path, 301/302 only), CmsMediaController (CRUD, `public-cms` disk separate from OPG, dimensions+sha256, multilingual), CmsMenuController (menu CRUD + nested menu items with translations, self-parent prevented, item scoped to menu), CmsSeoMetadataController (polymorphic per entity+locale upsert, idempotent, locale-isolated), CmsCommentController (index/moderate/destroy, 4 statuses), CmsContentPolicy (owner/tech_admin manage+delete; coordinator manage-only; patient/clinician denied). `public-cms` disk in `config/filesystems.php`. 37 CMS API routes registered (auth, NOT behind intake kill-switch). 22 API feature tests (CmsContentTest + CmsManagementTest). Fixed route-binding bug (`{tag}` binds to `$tag`). **CMS admin UI (§22) — all content types:** Posts (full CRUD + publish/unpublish; index with search/type/status filters, pagination; editor with multilingual fa/ar/en tabs, categories, tags, revisions). Categories (index/create/edit/update/delete, hierarchical parent, multilingual editor). Tags (index/inline-create/delete). Media (index/upload/delete with image validation + dimensions + sha256 on `public-cms` disk). Menus (index with create; per-menu item builder with parent select, URL, label; item/menu delete). Redirects (index/create/delete, 301/302, unique source_path validation). Comments (index with status filter; approve/mark-spam/delete; coordinator moderate-but-not-delete). **SEO metadata** (index with entity_type filter; per-post multilingual fa/ar/en editor: SEO title, meta description, canonical, robots directive, OG title/description, focus keyword; upsert per entity+locale). Authorization: manage = viewAny policy (owner/tech_admin/coordinator-editor), delete = owner/tech_admin only. Admin nav: posts/categories/tags/media/menus/redirects/comments/SEO. 36 admin translation keys added × 3 locales. Fix: `ValidationException` render respects web-vs-JSON (web redirects back with errors, API/JSON returns 422 JSON) — consistent with AuthenticationException fix. 20 admin UI tests (AdminCmsUiTest + AdminCmsContentUiTest + AdminCmsOpsUiTest + AdminCmsSeoUiTest). **Full suite: 131 passed (412 assertions).** | **WYSIWYG rich-text editor added** (vanilla-JS contenteditable toolbar: bold/italic/underline, H2/H3/paragraph, bullet/numbered lists, blockquote, link insertion; no npm dependency; hidden textarea remains form source; HtmlSanitizer stays security boundary). HtmlSanitizer **now wired** into post editor (sanitized_body computed via sanitize() on store/update); CMS **preview link** added (signed-URL, auth, noindex). | Browser QA of admin UI; no remaining code gaps. | Continue. |
| Role dashboards | IMPLEMENTED_UNVERIFIED (see row below) | Initially NOT_STARTED on `main`; **now implemented** — see the detailed Role dashboards row below for API + UI evidence. | Loading/error-state polish; per-role detail drilldown pages; mobile refinement. | Continue. |
| Public frontend | VERIFIED (local) | `Current Public_HTML/` + `Front-end v1/` static multilingual frontend; `public/home.blade.php` matches the deployed canonical lapis+apricot design (`backend/DESIGN.md` = runtime source of truth, `backend/public/assets/site.css`). **Design reconciliation resolved:** `Front-end v1/v3-preview/` (teal + `accepted-blue.css` override) is an earlier exploration that was NOT adopted; its `DESIGN.md` now carries a SUPERSEDED header so future agents do not reintroduce the rejected palette. The Blade views (home/blog/page) reuse the same canonical `site.css` + brand assets as deployed production. **Added:** `PageController::show` renders published CMS pages (type=Page) at `/{locale}/{slug}` and service pages (type=Service) at `/{locale}/services/{slug}`, with per-locale canonical, hreflang (fa/ar/en + x-default), OpenGraph, JSON-LD (`StructuredDataService::webPage()` / `service()`). **Blog index page** `/{locale}/blog/` (`BlogController::index`) lists published posts only with canonical + hreflang + pagination + empty state; added to per-locale sitemap (priority 0.9). **Localized custom 404** (`errors/404.blade.php`) detects locale from URL path segment, sets `<html lang dir>`, noindex. **Exception handler fix:** `bootstrap/app.php` `HttpExceptionInterface`/`ModelNotFoundException`/`AuthorizationException` renderers now return JSON only for `api/*` or `expectsJson()` requests, letting HTML requests fall through to Laravel's Blade error rendering (previously always returned JSON, so the 404 view never rendered for browsers). API 404/403 JSON unchanged. 23 SeoPublicTest cases (incl. blog index published-only, empty state, sitemap includes blog index, localized 404). | Browser QA of full public surface on PHP 8.3/MariaDB; BreadcrumbList/FAQPage when content lands. | Continue. |
...
| SEO | VERIFIED (local) | `SitemapController` (sitemap index `sitemap.xml` + per-locale `sitemap-{fa,ar,en}.xml` from published posts/pages/services only; excludes drafts/admin/api/private media). `RobotsController` (`robots.txt` allows `/`, disallows `/api/`,`/admin/`,`/cms/`,`/storage/`, references sitemap). `BlogController` renders published CMS posts at `/{locale}/blog/{slug}` with per-locale canonical, hreflang (fa/ar/en + x-default), robots directive, OpenGraph/Twitter Card, JSON-LD `schema_data` from `cms_seo_metadata`. Home view already had canonical/hreflang/meta-description/robots. **Added:** `PublicRedirectController` (fallback route) wires redirect manager into public 404 path — matches unmatched GET paths against `cms_redirects` (active only), issues 301/302 with hit-count increment, real 404 for unknown paths (no redirect loops). `CmsMediaServeController` (`/cms-media/{media}`) serves public CMS marketing media from `public-cms` disk with immutable cache headers, image-only guard (non-image → 404). Fixed BlogController OG image bug (`$seo->ogImage->path` → `route('cms.media.serve', ...)`, was using nonexistent `path` column instead of `storage_key`). 18 feature tests (SeoPublicTest): robots+sitemap ref, sitemap index, locale sitemap published-only, blog canonical+hreflang, draft 404, redirect 301, redirect 302, inactive redirect 404, unknown path 404, CMS media served, non-image media 404, **page renders WebPage schema, service renders Service schema, draft page 404, blog type isolation**, preview signed-URL (valid + patient rejected). | **Structured data done:** Organization JSON-LD on home (via @php+json_encode to avoid Blade @context directive collision); Article JSON-LD on blog show; **WebPage schema on CMS pages and Service schema on service pages** (provider=Organization "Royadarman", no fabricated medical claims) via `StructuredDataService::webPage()`/`service()`. BreadcrumbList/FAQPage deferred until real content/structure lands. | Continue. |
| Role dashboards | IMPLEMENTED_UNVERIFIED | **API:** `DashboardService` + `DashboardController` (`GET /api/v1/dashboard`, auth). Single dispatcher returns role-specific clinical/coordination payloads (NOT marketplace): **patient** (own cases/documents-count/has_published_review, open referral proposals, home-service requests, support conversations), **clinician** (credential_status + can_publish gate, assigned current review revisions via `whereDoesntHave('supersededBy')`, open drafts via no published publication event), **clinic rep** (owned clinics via active memberships, active referral grants with case status, pending proposals), **coordinator** (assigned case queue via `current_coordinator_id`, awaiting-patient count, open/unassigned support, home-service pending), **owner** (aggregate case-status counts, total/active clinics, verified practitioners, open support, published/pending CMS posts — no clinical detail), **tech admin** (outbox pending via `processed_at IS NULL`, failed notification deliveries, audit 24h, consent records, recent audit — no clinical data). 9 feature tests (DashboardTest). **UI (§13):** `Web\DashboardController` renders role-specific Blade views reusing `DashboardService`; `dashboard/layout.blade.php` (dark admin chrome, RTL/LTR, skip-link, noindex) + 6 role views (`patient`, `clinician`, `clinic_rep`, `coordinator`, `owner`, `tech_admin`) with stat cards, tables, status badges, empty/loading-style states; locale-aware route `/{locale}/dashboard` (SetLocale+auth) + `/dashboard` redirect; `dashboard` translation block (52 keys × fa/ar/en, parity verified). Fixed `AuthenticationException` render to redirect web guests (not JSON 401) while keeping API 401 JSON. 8 UI tests (DashboardUiTest): guest redirect, all 6 roles render role-specific content, Arabic locale render. **Full suite: 111 passed (357 assertions).** | Loading/error-state polish; per-role detail drilldown pages; mobile refinement. | Continue. |
...
8. Public frontend + SEO (sitemap, robots, hreflang, canonical, structured
   data, redirects, blog index, localized 404) keeping approved design.
   **Progress:** blog index page + localized 404 + exception-handler HTML/JSON
   split done; design reconciliation resolved (v3-preview marked superseded).
   200 tests pass (602 assertions). **Remaining:** BreadcrumbList/FAQPage when
   content structure lands; browser QA on PHP 8.3/MariaDB.
