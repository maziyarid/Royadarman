...
| CMS / Marketing | IMPLEMENTED_UNVERIFIED | 16 CMS tables (posts, translations, categories, tags, media, menus, seo_metadata, redirects, comments, revisions), 16 Eloquent models, PostStatus/PostType/CommentStatus/MenuItemType enums, HtmlSanitizer (no stored XSS), CmsPostController (CRUD+publish+unpublish), CmsPostPolicy (owner/tech_admin/coordinator-editor; patient/clinician denied), 8 post feature tests. **Added:** CmsCategoryController (index/show/store/update/destroy, hierarchical, self-parent prevented), CmsTagController (index/store/merge/destroy, tag-merge moves post_tag pivot), CmsRedirectController (index/store/update/destroy, 409 on duplicate source_path, 301/302 only), CmsMediaController (index/store/update/destroy, `public-cms` disk separate from OPG, dimensions+sha256, multilingual translations), CmsContentPolicy (owner/tech_admin manage; owner/tech_admin delete; coordinator manage-only). `public-cms` disk added to `config/filesystems.php`. 24 CMS routes registered (auth, NOT behind intake kill-switch). 12 feature tests (category CRUD+hierarchy, tag merge + self-merge-rejected, redirect duplicate + 301/302 + 410-rejected, media upload+dimensions+sha256, unsafe-type rejected, destroy removes file+record, patient denied). Fixed route-binding bug (`{tag}` must bind to `$tag`, not `$source`). | Build menu builder + SEO metadata + comment moderation controllers; admin UI; wire SEO to public frontend (sitemap/hreflang). | Continue. |
...
| Tests | VERIFIED (local) | 73 passed / 213 assertions across 15 files (OTP, case workflow, clinical access, document pipeline, locale+authz, operations, intake-disabled, error envelopes, **AuthorizationMatrixTest**, **SupportSystemTest**, **HomeServiceStatusTest**, **CmsPostTest**, **CmsContentTest**). PHP 8.4 local; Pint PASS; composer audit clean. | Run on PHP 8.3 prod target + MariaDB. | Run on prod. |
